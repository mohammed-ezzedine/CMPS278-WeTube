...
const EditChannel = () => {
  ...
  const handleSubmit = async (files, allFiles) => {
    ...
    let channelId = currentUser.channel.id;
    let userId = currentUser.id;
    let userSecret = currentUser.secret;
    data.append('image', files[0].file);
    try {
      let response = await put(
        `https://youtube278.azurewebsites.net/api/channel/${channelId}?userId=${userId}&userSecret=${userSecret}`,
        data,
        {
          headers: {
  ...
  return (
    <Container
      className="editChannel__container"
      component="main"
      maxWidth="xs"
    >
      <CssBaseline />
